...
- `tests/test_*.py` + `tests/run_all.py` — 70 stdlib-unittest tests (v3.13.0) covering `resolve_model`, `drive-sync-state`, `plugin-metadata`, `skill-line-check`, `connector_resolver`, the Hermes adapter (`plugin.yaml` + `__init__.py`), and the OpenClaw manifest. Run with `python tests/run_all.py`.
